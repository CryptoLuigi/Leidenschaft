...
from dislevel.mee6_export import get_mee6_data
from dislevel.utils import *
...
class LevelingSlash(commands.Cog):
    ...
    async def rank_user(self, interaction: Interaction, member: Member):
        ...
        try:
            ...
            await interaction.response.send_message(
                content=f"{interaction.user.mention} That user is unranked or a bot."
            )
    ...
    async def setbg(self, interaction: Interaction, *, url: str):
        ...
        await interaction.send(
            ephemeral=True, content="Background image has been updated"
        )
    ...
    async def resetbg(self, interaction: Interaction):
        ...
        await interaction.send(
            ephemeral=True, content="Background image has been set to default"
        )

    @slash_command(description="Set text color of your name in your rank card")
    async def setcolor(
        self, interaction: Interaction, *, color: str, color2: str, color3: str
    ):
        """Set color of the text of your rank card"""
        await set_text_color(
            self.bot, interaction.user.id, interaction.guild.id, color, color2, color3
        )
        await interaction.send(ephemeral=True, content="Text color has been updated")

    @slash_command(description="Toggle the overlay in your rank card(Default:On)")
    async def overlay(
        self,
        interaction: Interaction,
        toggle: int = SlashOption(name="toggle", choices={"On": 1, "Off": 0}),
    ):
        """Toggle the overlay in your rank card"""
        await toggle_overlay(
            self.bot, interaction.user.id, interaction.guild.id, state=toggle
        )
        await interaction.send(ephemeral=True, content="The overlay has been toggled")
    ...
    async def nick(
        ...
        await interaction.send(ephemeral=True, content="Your nickname has been toggled")
    ...
    async def resetcolor(self, interaction: Interaction):
        ...
        await interaction.send(
            ephemeral=True, content="Text color has been set to default"
        )
    ...
    async def resetrank(self, interaction: Interaction):
        ...
        async def areyousure(ctx):
            ...
            async def resetxp(ctx):
                ...
                await ctx.response.send_message(
                    f"{ctx.user.mention} has reset their rank."
                )
                await sync_level_roles(guild, member, 0)
            ...
            embed2 = Embed(
                title="Reset Rank",
                description='Are you **really** sure you want to reset your rank?\nLost XP and level cannot recovered afterwards.\nPress "Yes" if you want to proceed.\n',
                color=0x99C1F1,
            )

            await ctx.response.send_message(
                embed=embed2, view=warnview2, ephemeral=True
            )
        ...
        embed1 = Embed(
            title="Reset Rank",
            description='Are you sure you want to reset your rank?\nPress "Yes" if you want to proceed.\n',
            color=0x99C1F1,
        )
    ...
    async def sresetrank(self, interaction: Interaction, *, member: Member):
        ...
        async def areyousure(ctx):
            ...
            async def resetxp(ctx):
                ...
                await ctx.response.send_message(
                    f"{ctx.user.mention} has reset {member.mention}'s rank."
                )
                await sync_level_roles(guild, member, 0)
            ...
            embed2 = Embed(
                title="Reset Rank",
                description=f"Are you **really** sure you want to reset {member.mention}'s rank?",
                color=0x99C1F1,
            )
        ...
        embed1 = Embed(
            title="Reset Rank",
            description=f"Are you sure you want to reset {member.mention}'s rank?",
            color=0x99C1F1,
        )
    ...
    async def setfont(self, interaction: Interaction):
        ...
        async def my_callback(interaction):
            if int(select.values[0]) == 1:
                await set_text_font(
                    ...
                    interaction.user.id,
                    interaction.guild.id,
                    font=select.values[0],
                )
                await interaction.send(
                    ephemeral=True,
                    content="Text font has been updated",
                )
        ...
        await interaction.send(ephemeral=True, content="Choose font", view=view)
    ...
    async def add_server_bg(self, interaction: Interaction, *, url: str):
        ...
        await interaction.send(
            ephemeral=True, content="Background image has been added"
        )
    ...
    async def remove_server_bg(self, interaction: Interaction, *, name: str):
        ...
        await interaction.send(
            ephemeral=True, content="Background image has been deleted"
        )
    ...
    async def ex_mee6(self, interaction: Interaction, *, server_id: Optional[str]):
        ...
        await interaction.send(
            file=File(f"{interaction.guild.id}_leaderboard_full_out.txt")
        )
